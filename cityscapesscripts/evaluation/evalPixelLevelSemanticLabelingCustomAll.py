...
def evaluatePixelError(confMatrix, totalPixelCt):
    ...
    for label in args.evalLabels:
        ...
        if labelPixelSum <= 0.000 :
            ...
            sortedArrayIndices = np.flip(np.argsort(np.array(confMatrix[label, :])),axis=0)
            ct = 0;
...
def evaluateTrueVsOtherPixError(confMatrix, totalPixelCt) :
    ...
    #print(arr.shape)
    sorted2dArray = np.flip(np.dstack(np.unravel_index(np.argsort(arr.ravel()), confMatrix.shape)).squeeze(0),axis=0)
